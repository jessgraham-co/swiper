(function(){
  ...
  function fetchUserId(){
    ...
    .then(function(d){
      userId = d.user && (d.user.pk || d.user.id);
      if(!userId) throw new Error('no id');
      fetchFollowing();
    })
    .catch(function(){
      fetch('https://www.instagram.com/api/v1/friendships/0/following/?count=1', {
        credentials:'include',
        headers:{'X-CSRFToken':getCsrf(),'X-IG-App-ID':'936619743392459','X-Requested-With':'XMLHttpRequest'}
      })
      .then(function(r){ return r.json(); })
      .then(function(d){
        // Extract viewer id from meta tags or page source
        var metas = document.querySelectorAll('meta[name="author"]');
        ...
        for(var i=0;i<scripts.length;i++){
          try {
            ...
            var id = data && data.user_id || (data.config && data.config.viewerId);
            if(id){ userId = id; fetchFollowing(); return; }
          } catch(e){}
        }
        showError('Could not find your user ID. Open instagram.com, make sure you\'re logged in, then run the Shortcut again.');
      })
      .catch(function(){ showError('Network error. Are you on instagram.com and logged in?'); });
    });
  }
  ...
  function renderCurrent(){
    ...
    area.innerHTML = '';
    if(currentIdx + 1 < following.length){
      area.appendChild(buildCard(following[currentIdx+1], false));
    }
  ...
  function doSwipe(card, dir){
    ...
    setTimeout(function(){
      card.remove();
      if(currentIdx+1<following.length && !area.querySelector('[style*="scale(0.94)"]')){
        area.insertBefore(buildCard(following[currentIdx+1],false), area.firstChild);
      }
      if(currentIdx>=following.length){
